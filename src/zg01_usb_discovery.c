...
static void zg01_discover_all_alt_settings(struct usb_interface *interface)
{
    ...
    pr_debug("zg01_discovery: Discovering all alternate settings for interface %d\n",
    ...
    for (alt_idx = 0; alt_idx < interface->num_altsetting; alt_idx++) {
        ...
        pr_debug("zg01_discovery: === Alt Setting %d ===\n", info.alt_setting);
        pr_debug("zg01_discovery:   Endpoints: %d\n", info.num_endpoints);
        pr_debug("zg01_discovery:   Class: 0x%02x, SubClass: 0x%02x, Protocol: 0x%02x\n",
                altsetting->desc.bInterfaceClass,
                altsetting->desc.bInterfaceSubClass,
                altsetting->desc.bInterfaceProtocol);
        
        for (ep_idx = 0; ep_idx < info.num_endpoints && ep_idx < ARRAY_SIZE(info.endpoints); ep_idx++) {
            ...
            pr_debug("zg01_discovery:     EP 0x%02x: %s %s, MaxPacket=%d, Interval=%d%s\n",
                    ep_info->address,
...
int zg01_discover_usb_config(struct zg01_dev *dev)
{
    ...
    pr_debug("zg01_discovery: ========================================\n");
    pr_debug("zg01_discovery: USB Hardware Discovery for Yamaha ZG01\n");
    pr_debug("zg01_discovery: ========================================\n");
    
    /* Device-level information */
    pr_debug("zg01_discovery: Device: %04x:%04x (USB %d.%d)\n",
            le16_to_cpu(udev->descriptor.idVendor),
            le16_to_cpu(udev->descriptor.idProduct),
            (udev->descriptor.bcdUSB >> 8) & 0xff,
            udev->descriptor.bcdUSB & 0xff);
    
    pr_debug("zg01_discovery: Speed: %s\n", 
    ...
    if (udev->actconfig) {
        pr_debug("zg01_discovery: Current Configuration: %d (%d interfaces)\n",
    ...
    pr_debug("zg01_discovery: ========================================\n");
    pr_debug("zg01_discovery: Current Interface Analysis\n");
    pr_debug("zg01_discovery: ========================================\n");
    ...
    pr_debug("zg01_discovery: ========================================\n");
    pr_debug("zg01_discovery: Audio Endpoint Recommendations\n");
    pr_debug("zg01_discovery: ========================================\n");
    ...
    for (i = 0; i < interface->num_altsetting; i++) {
        ...
        for (ep_idx = 0; ep_idx < alt->desc.bNumEndpoints; ep_idx++) {
            ...
            if (is_audio_endpoint(ep_desc, &alt->desc)) {
                pr_debug("zg01_discovery: RECOMMENDED: Interface %d, Alt %d, EP 0x%02x (%s %s, %d bytes)\n",
                ...
                if ((ep_desc->bEndpointAddress & 0x0F) == 1) {
                    if (max_packet == 512) {
                        pr_debug("zg01_discovery:   -> MATCHES Voice channel from pcap (512 bytes)\n");
                    } else if (max_packet >= 8192) {
                        pr_debug("zg01_discovery:   -> MATCHES Game channel from pcap (8192 bytes)\n");
                    }
                }
            }
        }
    ...
    pr_debug("zg01_discovery: ========================================\n");
    pr_debug("zg01_discovery: Discovery Complete\n");
    pr_debug("zg01_discovery: ========================================\n");
...
static int __maybe_unused zg01_find_audio_endpoint(struct zg01_dev *dev, u8 *endpoint_addr, u8 *alt_setting)
{
    ...
    if (best_endpoint) {
        ...
        *alt_setting = best_alt;
        pr_debug("zg01_discovery: Selected endpoint 0x%02x (alt %d, %d bytes) for audio\n",
                best_endpoint, best_alt, best_packet_size);
        return 0;
    }
    
    pr_debug("zg01_discovery: No suitable audio endpoint found\n");
    return -ENODEV;
}
